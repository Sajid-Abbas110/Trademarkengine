...
export default function Step8Payment({ packageType, addSearch, processingSpeed, onBack, onSubmit }: Step8PaymentProps) {
    ...
    // Calculate costs
    const packagePrice = packageType === 'basic' ? 99 : 199;
    const filingFee = 85;
    const searchPrice = addSearch ? 99 : 0;
    const speedPrice = processingSpeed === 'express' ? 50 : 0;

    const subtotal = packagePrice + filingFee + searchPrice + speedPrice;
                        ...
                        <div className="space-y-4 text-xs font-semibold text-slate-700 pb-6 border-b border-slate-300/50 mb-6">
                            ...
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>Copyright Package - {packageType === 'basic' ? 'Basic' : 'Deluxe'}</span>
                                </div>
                                <span>${formatPrice(packagePrice)}</span>
                            </div>

                            <div className="flex justify-between items-start">
                                <span>Copyright USCO Filing Fee</span>
                                <span>${formatPrice(filingFee)}</span>
                            </div>
                            ...
                            {addSearch && (
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-2">
                                        <button onClick={onBack} className="text-[#ea580c] hover:underline font-bold text-[10px]">Edit</button>
                                        <span>Comprehensive Search Report</span>
                                    </div>
                                    <span>${formatPrice(searchPrice)}</span>
                                </div>
                            )}
                            ...
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-2">
                                    <button onClick={onBack} className="text-[#ea580c] hover:underline font-bold text-[10px]">Edit</button>
                                    <span>{processingSpeed === 'standard' ? 'Standard Processing' : 'Express Processing'}</span>
                                </div>
                                <span>${formatPrice(speedPrice)}</span>
                            </div>
